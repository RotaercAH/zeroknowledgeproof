...
public final class ElGamalPrivateKey implements ElGamal_Key, Serializable, PrivateKey, Runnable, CipherConstants
{
	...
	public final Map <BigInteger, BigInteger> LUT;
	...
	public ElGamalPrivateKey(BigInteger p, BigInteger x, BigInteger g, BigInteger h, BigInteger y, boolean ADDITIVE)
	{
		this.p = p;
		this.x = x;
		this.g = g;
	...
	private void decrypt_table() 
	{
		...
		//BigInteger decrypt_size = FIELD_SIZE.add(FIELD_SIZE).subtract(TWO).add(TWO.pow(32));
		BigInteger decrypt_size = FIELD_SIZE.add(FIELD_SIZE).subtract(TWO).add(TWO.pow(20));
