...
public class ElGamalCipher extends CipherSpi
{
			...
			byte[] output, int outputOffset)
	{
		...
		byte [] messageBytes = decrypt(new ElGamal_Ciphertext(gr, hrgm, r), (ElGamalPrivateKey) keyElGamal).toByteArray();
	...
	private static ElGamal_Ciphertext Encrypt(BigInteger plaintext, ElGamalPublicKey pk)
	{
		...
		return new ElGamal_Ciphertext(gr, hrgm, r);
	}
	...
	private static ElGamal_Ciphertext Encrypt_Homomorph(BigInteger plaintext, ElGamalPublicKey pk) 
	{
		BigInteger pPrime = pk.p.subtract(BigInteger.ONE).divide(ElGamalKeyPairGenerator.TWO);
		BigInteger r = NTL.RandomBnd(pPrime);
		BigInteger yr = pk.y.modPow(r, pk.p);
		BigInteger gm = pk.g.modPow(plaintext, pk.p);
		return new ElGamal_Ciphertext(pk.h.modPow(r, pk.p), yr.multiply(gm).mod(pk.p), r);
	}
	...
	{
		BigInteger hr = ciphertext.hr.modPow(sk.x, sk.p);
		return ciphertext.gmyr.multiply(hr.modInverse(sk.p)).mod(sk.p);
	}
	...
	private static BigInteger Decrypt_Homomorph(ElGamal_Ciphertext ciphertext, ElGamalPrivateKey sk) 
	{
		...
		BigInteger hrx_yr = ciphertext.hr.modPow(sk.x, sk.p);
		// g^m = (h^r * g^m) * (h^r)-1 (mod p) = g^m (mod p)
		BigInteger gm = ciphertext.gmyr.multiply(hrx_yr.modInverse(sk.p)).mod(sk.p);
	...
	public static ElGamal_Ciphertext multiply_scalar(ElGamal_Ciphertext ciphertext1, BigInteger scalar, ElGamalPublicKey pk)
	{
		if(pk.ADDITIVE)
		{
			...
			BigInteger q = pk.p.subtract(BigInteger.ONE).divide(BigInteger.valueOf(2));
			answer = new ElGamal_Ciphertext(ciphertext1.hr.modPow(scalar, pk.p), ciphertext1.gmyr.modPow(scalar, pk.p), ciphertext1.r.multiply(scalar).mod(q));
			return answer;
		}
		else
		{
	...
	public static ElGamal_Ciphertext multiply(ElGamal_Ciphertext ciphertext1, ElGamal_Ciphertext ciphertext2, ElGamalPublicKey pk)
	{
		...
		{
			ElGamal_Ciphertext answer = null;
			answer = new ElGamal_Ciphertext(ciphertext1.hr.multiply(ciphertext2.hr).mod(pk.p),
					ciphertext1.gmyr.multiply(ciphertext2.gmyr).mod(pk.p), r);
			return answer;	
		}
	}
	...
	public static ElGamal_Ciphertext divide(ElGamal_Ciphertext ciphertext1, ElGamal_Ciphertext ciphertext2, ElGamalPublicKey pk)
	{
		...
		else
		{
			...
			// Get mod inverse
			BigInteger inv_gr = ciphertext2.hr.modInverse(pk.p);
			BigInteger inv_mhr = ciphertext2.gmyr.modInverse(pk.p);
			neg_ciphertext2 = new ElGamal_Ciphertext(inv_gr, inv_mhr, r);
			// multiply
	...
	public static ElGamal_Ciphertext add(ElGamal_Ciphertext ciphertext1, ElGamal_Ciphertext ciphertext2, ElGamalPublicKey pk)
	{
		if(pk.ADDITIVE)
		{
			...
			BigInteger q = pk.p.subtract(BigInteger.ONE).divide(BigInteger.valueOf(2));
			answer = new ElGamal_Ciphertext(ciphertext1.hr.multiply(ciphertext2.hr).mod(pk.p),
					ciphertext1.gmyr.multiply(ciphertext2.gmyr).mod(pk.p), ciphertext1.r.add(ciphertext2.r).mod(q));
			return answer;	
		}
		else
		{
	...
	public static List<byte[]> encodeCiphertext(ElGamal_Ciphertext ciphertext){
		List<byte[]> result = new ArrayList<>();
		result.add(HexUtil.tobyte32(ciphertext.gmyr.toByteArray()));
		result.add(HexUtil.tobyte32(ciphertext.hr.toByteArray()));
		result.add(HexUtil.tobyte32(ciphertext.r.toByteArray()));
		if(result.size()!=3){
			System.out.println("密文编码大小不为3");
			System.out.println(ciphertext.gmyr);
			System.out.println(ciphertext.hr);
	...
	public static ElGamal_Ciphertext decodeCiphertext(List<byte[]> list){
		List<byte[]> temp = new ArrayList<>();
		return new ElGamal_Ciphertext(new BigInteger(list.get(1)), new BigInteger(list.get(0)), new BigInteger(list.get(2)));
	}
